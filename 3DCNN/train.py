...
from utils.record import drawFig, saveConfig
...
def main(args):
    ...
    print(
        "dataset loaded, train set size", len(train_set), "test set size", len(test_set)
    )
    ...
    save_best = tf.keras.callbacks.ModelCheckpoint(
        filepath=os.path.join(savingPath, "best_model.hdf5"),
        monitor=monitor,
        verbose=1,
        save_best_only=True,
    )
    save_latest = tf.keras.callbacks.ModelCheckpoint(
        filepath=os.path.join(savingPath, "latest_model.hdf5"),
        monitor=monitor,
        verbose=1,
    ...
    history = model.fit_generator(
        generator=gen(train_set, args.batch, shuffle=True, augment=args.augment),
        epochs=args.epoch,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--datapath",
        type=str,
        default="./data/example_dataset/distance",
        help="Path to tensors",
    )
    parser.add_argument(
        "--weights", type=str, default="", help="Continue training based on weights"
    )
    parser.add_argument(
        "--savingPath",
        type=str,
        default="./models/example_model",
    ...
    parser.add_argument(
        "--train_set",
        type=str,
        default="./data/example_dataset/part_0_train.csv",
        help="Path to train set csv file",
    )
    parser.add_argument(
        "--test_set",
        type=str,
        default="./data/example_dataset/part_0_val.csv",
        help="Path to val set csv file",
    )
